...
import interactive.view.postcardorg.PostCardUtility;
		...
		for (int i = 0; i < jsonArrayPostcard.length(); ++i)
		{
			...
			if (parseJsonHeader(jsonPostcard, jsonHeader) && parseJsonPostcard(jsonPostcard, jsonBody))
			{
				if (jsonHeader.mbIsVisible)
				{
					...
					postcard.initPostcardFrame(jsonHeader.mstrName, getScaleUnit(jsonHeader.mnX),
							getScaleUnit(jsonHeader.mnY), getScaleUnit(jsonHeader.mnWidth),
							getScaleUnit(jsonHeader.mnHeight));

					postcard.addPostcard(strBookPath + jsonBody.mstrSrcBack);
					postcard.addPostcard(strBookPath + jsonBody.mstrSrcFront);
